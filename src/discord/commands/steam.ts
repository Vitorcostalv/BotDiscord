...
export const steamCommand = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName('link')
        .setDescription('Vincula um SteamID64 ao usuario')
        .addStringOption((option) =>
          option
            .setName('steamid64')
            .setDescription('SteamID64 (17 digitos)')
            .setRequired(true),
        )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName('unlink')
        .setDescription('Remove o vinculo Steam')
        .addUserOption((option) => option.setName('user').setDescription('Usuario alvo').setRequired(false)),
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName('view')
        .setDescription('Mostra o perfil Steam vinculado')
        .addUserOption((option) => option.setName('user').setDescription('Usuario alvo').setRequired(false)),
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName('refresh')
        .setDescription('Atualiza o cache Steam')
        .addUserOption((option) => option.setName('user').setDescription('Usuario alvo').setRequired(false)),
    ),
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      if (subcommand === 'link') {
        ...
        if (existing && !force) {
          const embed = createSuziEmbed('warning')
            ...
            .setDescription('Use /steam link force:true para sobrescrever.');
          await safeRespond(interaction, { embeds: [embed] });
          return;
        }
      ...
      if (subcommand === 'view' || subcommand === 'refresh') {
        const targetUser = interaction.options.getUser('user') ?? interaction.user;
        const link = getSteamLink(targetUser.id);
        if (!link) {
          const embed = createSuziEmbed('warning')
            .setTitle('Nenhum Steam vinculado')
            .setDescription('Use /steam link para vincular um SteamID64.');
          await safeRespond(interaction, { embeds: [embed] });
          return;
        }

        const summaryResult = await getCachedSummary(link.steamId64, { force: subcommand === 'refresh' });
        if (!summaryResult.ok) {
          const embed = createSuziEmbed('warning')
      ...
      logError('SUZI-CMD-002', error, { message: 'Erro no comando /steam', subcommand });
      const embed = createSuziEmbed('warning')
